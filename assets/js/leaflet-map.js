...
function createAssignPopup(lat, lng) {
  ...
  const sectorLabel = allowedSectors.join(" / ") || "الكل";

  // Custom searchable dropdown HTML + Inline CSS for the popup
  return `
        <style>
            .vol-search-container { width: 280px; font-family: 'Cairo', sans-serif; }
            ...
            .vol-sector { font-size: 10px; padding: 2px 6px; border-radius: 4px; background: #e0e7ff; color: #3730a3; font-weight: bold; margin-left: 4px; }
            ...
            .sector-badge { display: inline-block; padding: 4px 12px; background: #dbeafe; color: #1e40af; border-radius: 8px; font-size: 12px; font-weight: bold; margin-bottom: 8px; }
        </style>
        <div class="vol-search-container" dir="rtl text-right">
            <h3 class="font-bold text-lg mb-2 text-primary text-center">تعيين متطوع</h3>
            <div class="text-center mb-3">
                <span class="sector-badge">قطاع ${sectorLabel}</span>
            </div>
...
// Helper to render options with sector badge
    ...
    .map(
      (v) => `
        <div 
            ...
            <div>
                <span class="vol-sector">${v.sector || "-"}</span>
...
/**
 * Filter markers by sector (A, B, C, D)
 * @param {string} sector - Sector to filter ('all', 'A', 'B', 'C', 'D')
 */
export async function filterBySector(sector) {
  currentSectorFilter = sector;
  currentLocationFilter = "all"; // Reset location filter
  await refreshMarkersWithFilter();
}

/**
 * Filter markers by location type (gate, inforoom)
 * @param {string} locationType - 'all', 'gate', 'inforoom'
 */
export async function filterByLocation(locationType) {
  currentLocationFilter = locationType;
  currentSectorFilter = "all"; // Reset sector filter
  await refreshMarkersWithFilter();
}
...
async function refreshMarkersWithFilter() {
  ...
  // Ensure map container size is correctly calculated
  map.invalidateSize();
  ...
  // Get occupied volunteers for current hall
  let occupied = await getOccupiedVolunteers(currentHall);
  ...
  if (currentLocationFilter === "gate") {
    occupied = occupied.filter(
      (v) => v.hall_id == 101 || v.current_loc === "101",
    );
  } else if (currentLocationFilter === "inforoom") {
    occupied = occupied.filter(
      (v) => v.hall_id == 102 || v.current_loc === "102",
    );
  }

  // Create markers for each filtered volunteer
  occupied.forEach((volunteer) => {
    if (volunteer.current_loc) {
      const coords = parseLocationCoords(volunteer.current_loc);
      if (coords) {
  ...
  // Update filter count display if exists
  const filterCountEl = document.getElementById("filter-count");
  if (filterCountEl) {
    filterCountEl.textContent = `عدد النتائج: ${Object.keys(markers).length}`;
  }
}
